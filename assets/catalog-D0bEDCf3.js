import{s as b,c as p,e as c,u as E,t as P,A as T}from"./utils-BWzLDlxE.js";class ${static async getStoreConfig(){const{data:e,error:a}=await b.from("store_config").select("*").eq("id",1).maybeSingle();if(a)throw a;return{id:1,loja:(e==null?void 0:e.loja)??"Imagine Art",whatsapp:(e==null?void 0:e.whatsapp)??"",mensagem:(e==null?void 0:e.mensagem)??"Olá! Gostaria de fazer este pedido:",moeda:(e==null?void 0:e.moeda)??"BRL",banner:(e==null?void 0:e.banner)??"Escolha seus produtos personalizados.",delivery_lead_days:Math.max(1,Number((e==null?void 0:e.delivery_lead_days)??5))}}static async getProducts(){const{data:e,error:a}=await b.from("products").select("*").eq("active",!0).order("created_at",{ascending:!1});if(a)throw a;return(e??[]).map(i=>({...i,custom_fields:Array.isArray(i.custom_fields)?i.custom_fields:[]}))}static async getSchools(){const{data:e,error:a}=await b.from("schools").select("*").eq("active",!0).order("name",{ascending:!0});if(a)throw a;return e??[]}static async getPaymentMethods(){const{data:e,error:a}=await b.from("payment_methods").select("*").eq("active",!0).order("name",{ascending:!0});if(a)throw a;return e??[]}}const n={config:{loja:"Imagine Art",whatsapp:"",mensagem:"Olá! Gostaria de fazer este pedido:",moeda:"BRL",banner:""},products:[],schools:[],paymentMethods:[],cart:[]},q=document.querySelector("#app");function x(){q.innerHTML=`
  ...
  `}const o=t=>document.getElementById(t);function m(t,e="ok"){const a=o("orderStatus");a.classList.remove("hidden"),a.className=`status ${e}`,a.textContent=t}function y(){const e=o("cPagamento").selectedOptions[0];return Number((e==null?void 0:e.dataset.fee)||0)}function w(){return n.cart.reduce((t,e)=>t+e.sale_price*e.qtd,0)}function C(){return w()+y()}function L(){o("cartCount").textContent=`${n.cart.reduce((t,e)=>t+e.qtd,0)} item(s)`,o("cartTotal").textContent=p(C(),n.config.moeda)}function j(t=""){return'<option value="">Selecione a escola</option>'+n.schools.map(e=>`
    <option value="${c(e.name)}" ${e.name===t?"selected":""}>${c(e.name)}</option>
  `).join("")}function z(){const t=o("filtroCategoria"),e=[...new Set(n.products.map(a=>a.category||"Sem categoria"))];t.innerHTML='<option value="">Todas as categorias</option>'+e.map(a=>`<option value="${c(a)}">${c(a)}</option>`).join("")}function M(){const t=o("buscaCatalogo").value.trim().toLowerCase(),e=o("filtroCategoria").value;return n.products.filter(a=>{const i=[a.name,a.category||"",a.description||""].join(" ").toLowerCase().includes(t),r=!e||(a.category||"Sem categoria")===e;return i&&r})}function _(){const t=o("catalogGrid"),e=M();if(!e.length){t.innerHTML='<div class="card"><strong>Nenhum produto disponível.</strong><div style="margin-top:6px;color:#6b7280">Cadastre produtos no admin para exibir aqui.</div></div>';return}t.innerHTML=e.map(a=>{const r=(a.custom_fields||[]).map((l,s)=>`
      <div>
        <label>${c(l)}</label>
